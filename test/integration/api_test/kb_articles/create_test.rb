...
class CreateArticleTest < Redmine::ApiTest::Base
  ...
  def test_create_article_with_uploaded_file
    ...
    get "/projects/1/knowledgebase/articles/#{article.id}.xml?include=attachments", :headers => credentials('admin')
  ...
  def test_create_article_with_multiple_uploaded_files_as_json
    ...
    assert_difference 'KbArticle.count' do
      post(
        ...
        :params => payload,
        :headers => {"CONTENT_TYPE" => 'application/json'}.merge(credentials('admin')))
      assert_response :created
    end
  ...
  def assert_creates_article(params = {})
    assert_difference('KbArticle.count') do
      post(
        ...
        :headers => {"CONTENT_TYPE" => "application/#{params[:format]}"}.merge(credentials('admin')))  
    end
